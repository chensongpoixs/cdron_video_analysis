...
purpose:		log
************************************************************************************************/
#ifndef C_GPU_DETECTOR_H
#define C_GPU_DETECTOR_H
#include <thread>
#include <string>
#include <functional>
#if defined(_MSC_VER)
...
namespace chen
{
	...
	public:
		 ...
		 */
		cgpu_detector(const std::string& model_path, const torch::DeviceType& device_type);
	...
	private:
		 ...
		 */
		static std::vector<float> LetterboxImage(const cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst, const cv::Size& out_size = cv::Size(640, 640));
