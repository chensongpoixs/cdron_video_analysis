...
#include "cgpu_detector.h"
namespace chen {

	cgpu_detector::cgpu_detector(const std::string& model_path, const torch::DeviceType& device_type) : device_(device_type) {
		try {
	...
	std::vector<std::vector<CDetection>>
		...
		cv::cuda::GpuMat img_input = img.clone();
		//384x640
		...
		at::Tensor  tensor_img = torch::from_blob(img_input.data, { 1, img_input.rows, img_input.cols, img_input.channels() }).to(device_);

		//// python at::IntArrayRef
		//at::IntArrayRef imgsz{1, 3, 640, 640};
	...
	std::vector<float> cgpu_detector::LetterboxImage(const cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst, const cv::Size& out_size) {
	...
	torch::Tensor cgpu_detector::xywh2xyxy(const torch::Tensor& x) {
	...
	void cgpu_detector::Tensor2Detection(const at::TensorAccessor<float, 2>& offset_boxes,
