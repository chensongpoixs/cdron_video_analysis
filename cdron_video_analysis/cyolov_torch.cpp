...
namespace chen {
	...
	cyolov_torch::cyolov_torch(std::string ptFile, std::string version  , std::string devices  , bool isHalf , int height  , int width  , float confThres  , float iouThres)
		:device   (devices == "cpu" ? torch::kCPU : torch::kCUDA)
	{
	...
	cv::Scalar cyolov_torch::getRandScalar()
	{
		return cv::Scalar(std::rand() % 256, std::rand() % 256, std::rand() % 256);
	}

	cv::Mat cyolov_torch::img2RGB(cv::Mat img)
	{
	...
	torch::Tensor cyolov_torch::img2Tensor(cv::Mat img)
	{
	...
	torch::Tensor cyolov_torch::xywh2xyxy(torch::Tensor x)
	{
	...
	torch::Tensor cyolov_torch::nms(torch::Tensor bboxes, torch::Tensor scores, float thresh)
	{
	...
	std::vector<torch::Tensor> cyolov_torch::sizeOriginal(std::vector<torch::Tensor> result, std::vector<ImageResizeData> imgRDs)
	{
	...
	std::vector<torch::Tensor> cyolov_torch::prediction(torch::Tensor data)
	{
		if (this->isHalf)
		{
	...
	std::vector<torch::Tensor> cyolov_torch::prediction(std::string filePath)
	{
		cv::Mat img = cv::imread(filePath);
		return prediction(img);
	}

	std::vector<torch::Tensor> cyolov_torch::prediction(cv::Mat img)
	{
	...
	ImageResizeData cyolov_torch::resize(cv::Mat img, int height, int width)
	{
	...
	ImageResizeData cyolov_torch::resize(cv::Mat img)
	{
		return cyolov_torch::resize(img, height, width);
	}

	std::vector<ImageResizeData> cyolov_torch::resize(std::vector<cv::Mat> imgs, int height, int width)
	{
		std::vector<ImageResizeData> imgRDs;
		for (int i = 0; i < imgs.size(); i++)
		{
			imgRDs.push_back(cyolov_torch::resize(imgs[i], height, width));
		}
		return imgRDs;
	}

	std::vector<ImageResizeData> cyolov_torch::resize(std::vector<cv::Mat> imgs)
	{
		return cyolov_torch::resize(imgs, height, width);
	}

	std::vector<cv::Mat> cyolov_torch::drawRectangle(std::vector<cv::Mat> imgs, std::vector<torch::Tensor> rectangles, std::map<int, std::string> labels, int thickness)
	{
		std::map<int, cv::Scalar> colors;
		return drawRectangle(imgs, rectangles, colors, labels, thickness);
	}

	std::vector<cv::Mat> cyolov_torch::drawRectangle(std::vector<cv::Mat> imgs, std::vector<torch::Tensor> rectangles, int thickness)
	{
		std::map<int, cv::Scalar> colors;
	...
	std::vector<cv::Mat> cyolov_torch::drawRectangle(std::vector<cv::Mat> imgs, std::vector<torch::Tensor> rectangles, std::map<int, cv::Scalar> colors, std::map<int, std::string> labels, int thickness)
	{
	...
	cv::Mat cyolov_torch::drawRectangle(cv::Mat img, torch::Tensor rectangle, int thickness)
	{
		std::map<int, cv::Scalar> colors;
		std::map<int, std::string> labels;
	...
	cv::Mat cyolov_torch::drawRectangle(cv::Mat img, torch::Tensor rectangle, std::map<int, std::string> labels, int thickness)
	{
		std::map<int, cv::Scalar> colors;
		return drawRectangle(img, rectangle, colors, labels, thickness);
	}

	cv::Mat cyolov_torch::drawRectangle(cv::Mat img, torch::Tensor rectangle, std::map<int, cv::Scalar> colors, std::map<int, std::string> labels, int thickness)
	{
	...
	bool cyolov_torch::existencePrediction(torch::Tensor clazz)
	{
		return clazz.size(0) > 0 ? true : false;
	}

	bool cyolov_torch::existencePrediction(std::vector<torch::Tensor> classs)
	{
